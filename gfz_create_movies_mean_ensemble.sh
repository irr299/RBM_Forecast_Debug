...
/usr/bin/ffmpeg -y -f concat -safe 0 -i "$FRAME_LIST_FILE" -r 30 -vf "scale=1920:1080:flags=lanczos" -c:v libx264 -preset medium -crf 23 -pix_fmt yuv420p -movflags +faststart -bf 0 Forecast_UTC_E_1_MeV_PA_50_latest_mean_ensemble.mp4
rm -f "$FRAME_LIST_FILE"
