...
function  [sw,mag] = get_realtime_ace_sw_mag_from_mat(path_ace,matlabd,prop,time_start,time_end)
    ...
    if prop
        ...
        sw.matlabd=matlabd;
        sw.jd=interp1(sw_prop(:,1),sw_prop(:,2),sw.matlabd,'linear');
        sw.stat=interp1(sw_prop(:,1),sw_prop(:,3),sw.matlabd,'nearest');
        sw.swn=interp1(sw_prop(:,1),sw_prop(:,4),sw.matlabd,'linear');
        sw.swv=interp1(sw_prop(:,1),sw_prop(:,5),sw.matlabd,'linear');
        sw.swT=interp1(sw_prop(:,1),sw_prop(:,6),sw.matlabd,'linear');

        mag.matlabd=matlabd;
        mag.jd=interp1(mag_prop(:,1),mag_prop(:,2),mag.matlabd,'linear');
        mag.stat=interp1(mag_prop(:,1),mag_prop(:,3),mag.matlabd,'nearest');
        mag.bx=interp1(mag_prop(:,1),mag_prop(:,4),mag.matlabd,'linear');
        mag.by=interp1(mag_prop(:,1),mag_prop(:,5),mag.matlabd,'linear');
        mag.bz=interp1(mag_prop(:,1),mag_prop(:,6),mag.matlabd,'linear');
        mag.bt=interp1(mag_prop(:,1),mag_prop(:,7),mag.matlabd,'linear');
        mag.lat=interp1(mag_prop(:,1),mag_prop(:,8),mag.matlabd,'linear');
        mag.lon=interp1(mag_prop(:,1),mag_prop(:,9),mag.matlabd,'linear');
